$(document).ready(function() {
	...
	socket.on('nickChange', function (user) {
		chatcontent.append('<p><span style="color:' + user.color + '">' + user.oldNick + '</span> changed their nick to <span style="color:' + user.color + '">' + user.newNick + '</span></p>');
		chatScrollDown();
	});
	...
	thickness.change(function() {
		thickness.val(Math.max(1, Math.min(30, thickness.val())));
		px = thickness.val();
	});
